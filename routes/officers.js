...
router.get("/officerscollection/new", middleware.isLoggedIn, function(req, res){
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    officerPicture.mv("./public/images/" + req.files.officerPicture.name, function(err){
        if(err){
            console.log(err);
        };
        console.log('success');
    ...
    var newOfficer = {title: title, name: name, callsign: callsign, pictureLoc: pictureLoc};
...
router.get("/officerscollection/:id", middleware.isLoggedIn, function(req, res){
...
router.put("/officerscollection/:id", middleware.isLoggedIn, function(req, res){
    Officers.findByIdAndUpdate(req.params.id, req.body, function(err){
        if(err){
            req.flash("error", "something went wrong");
            res.redirect("back");
        } else {
...
router.delete("/officerscollection/:id", middleware.isLoggedIn, function(req, res){
